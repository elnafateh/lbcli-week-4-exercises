# Create a raw transaction and add this message in it: "btrust builder 2026"

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
 transaction=""
decoded_transaction=$(bitcoin-cli -regtest decoderawtransaction "$transaction")
transaction_id=$(echo "$decoded_transaction" | jq -r '.txid')

# Hex of "btrust builder 2026" (createrawtransaction wraps this in OP_RETURN)
op_return_data=$(printf '%s' "btrust builder 2026" | od -An -tx1 | tr -d ' \n')

# Create a transaction with an OP_RETURN output followed by a payment output
bitcoin-cli -regtest -named createrawtransaction \
  inputs='''[
    { "txid": "'"$transaction_id"'", "vout": 0 },
    { "txid": "'"$transaction_id"'", "vout": 1 }
  ]''' \
  outputs='''[
    { "data": "'"$op_return_data"'" },
    { "2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP": 0.2 }
  ]'''
